...
const common_ancestor_path_1 = require("common-ancestor-path");
...
async function compile(configuration, data, uri) {
    ...
    const relativeUri = node_path_1.default.relative(configuration.projectRoot, uri);
    ...
    const implicitIntegrationFolder = (0, assertions_1.ensure)((0, common_ancestor_path_1.commonAncestorPath)(...(0, find_cypress_specs_1.getSpecs)(configuration, "foobar", true)
        .map(node_path_1.default.dirname)
        .map(node_path_1.default.normalize)), "Expected to find a common ancestor path");
    const preprocessor = await (0, preprocessor_configuration_1.resolve)(configuration, (0, node_1.getEnv)(configuration), implicitIntegrationFolder);
    const { stepDefinitions } = preprocessor;
    (0, debug_1.default)(`resolving step definitio|ns using template(s) ${(0, node_util_1.inspect)(stepDefinitions)}`);
    ...
    (0, debug_1.default)(`for ${(0, node_util_1.inspect)((0, paths_1.ensureIsRelative)(configuration.projectRoot, uri))} yielded patterns ${(0, node_util_1.inspect)(stepDefinitionPatterns)}`);
    ...
    if (stepDefinitionPaths.length === 0) {
        ...
        (0, debug_1.default)(`found step definitions ${(0, node_util_1.inspect)(stepDefinitionPaths.map((path) => (0, paths_1.ensureIsRelative)(configuration.projectRoot, path)))}`);
    }
    const prepareLibPath = (...parts) => stringify(node_path_1.default.join(__dirname, ...parts));
